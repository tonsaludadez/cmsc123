...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (isValidNode()) {
      SibTreeNode temp = new SibTreeNode(myTree, item);
      temp.parent = this;
      if (c <= 1) {
        temp.nextSibling = firstChild;
        firstChild = temp;
        ...
        SibTreeNode kid = firstChild;
        int i = 1;
          while ((kid.nextSibling != null) && (i < c - 1)) {
            kid = kid.nextSibling;
            i++;
          }

          temp.nextSibling = kid.nextSibling;
          kid.nextSibling = temp;
      }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (isValidNode()) {
      if (myTree.root() == this && myTree.root().isValidNode()){
        valid = false;
        myTree.size--;
      }
        ...
        if (this.parent.firstChild == this) {
          ...
          SibTreeNode trav = parent.firstChild;
          while (trav.nextSibling != this){
            trav = trav.nextSibling;
          }

          trav.nextSibling = nextSibling;
        }
      valid = false;
      myTree.size--;
    }
